...
WORD DEFAULT_COLOR = FOREGROUND_RED |
                     FOREGROUND_GREEN|
                     FOREGROUND_BLUE ;

WORD BRIGHT_GREEN_COLOR = FOREGROUND_GREEN |
                          FOREGROUND_INTENSITY;

// bright-red FG white BG
WORD UNSELECTED_COLOR = FOREGROUND_RED |
                        FOREGROUND_INTENSITY |
                        BACKGROUND_RED |
                        BACKGROUND_BLUE |
                        BACKGROUND_GREEN;
// white FG red BG
WORD SELECTED_COLOR = FOREGROUND_BLUE |
                      FOREGROUND_GREEN |
                      FOREGROUND_RED |
                      BACKGROUND_RED;
// bright-blue FG blue BG
WORD CARDBACK_COLOR = FOREGROUND_BLUE |
                      BACKGROUND_RED |
                      BACKGROUND_BLUE |
                      BACKGROUND_GREEN;
// White BG
WORD WHITEBACK_COLOR = BACKGROUND_INTENSITY|
                        BACKGROUND_RED |
                        BACKGROUND_BLUE |
                        BACKGROUND_GREEN;
// black FG
WORD BLACKLETTER_COLOR = BACKGROUND_INTENSITY|
                        BACKGROUND_RED |
                        BACKGROUND_BLUE |
                        BACKGROUND_GREEN;
// red FG
WORD REDLETTER_COLOR = FOREGROUND_RED|
...
void Clean(){
    for (int i = 0;
         i < CARDSBOARD_TOP_EDGE + cardsFrameY1 + yPerFrame - 3;
         i++) {
        gotoxy(0, 0 + i);
        for (int j = 0;
             j < CARDSBOARD_LEFT_EDGE + cardsFrameX1 + xPerFrame - 3;
             j++){
            cout << " ";
        }
    }
...
void InitGameUI() {
    ...
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),
                            DEFAULT_COLOR);
    //set message frame
    ShowAFrame(MESSAGE_LEFT_EDGE, MESSAGE_TOP_EDGE,
                MESSAGE_WIDTH,
    ...
    for (int i = 0; i < CARDS_WIDTH; i++)
    {
        ...
        {
            //set cards
            ShowACard(stackCards[i][j].xCard,
                      stackCards[i][j].yCard,
                      false);
    ...
    while(!isOver){
        ...
        while(true){
            ...
            if(stackCards[pos.X][pos.Y].ifEmpty)
                continue;
            ShowACard(stackCards[pos.X][pos.Y].xCard,
                        stackCards[pos.X][pos.Y].yCard,
                        true,
                        stackCards[pos.X][pos.Y].color,
                        stackCards[pos.X][pos.Y].num);
            if (tempCard1.X == -1)
                tempCard1 = pos;
